...
def run_ann_experiment(A_norm, B_norm):
    pairs, _, _ = ANN_injective.match_approx_nn_injective(A_norm, B_norm, k=50, strict=False)
    return pairs


def run_partial_ot_safe(A_norm, B_norm, filename):
    n_a = len(A_norm)
    n_b = len(B_norm)
    ...
    try:
        ...
        if faiss.get_num_gpus() > 0:
            cpu_index = faiss.IndexFlatL2(B_norm.shape[1])
            index = faiss.index_cpu_to_gpu(res, 0, cpu_index)
        else:
            index = faiss.IndexFlatL2(B_norm.shape[1])
            ...
            for i in range(REPEAT_TIMES):
                ...
                try:
                    if algo_name == "Partial_OT":
                        pairs = algo_func(A_norm, B_norm, filename)
                    else:
